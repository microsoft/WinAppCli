<#
.SYNOPSIS
Test script for the wpf-app sample.

.DESCRIPTION
Builds the WPF app in Release mode, which triggers the automatic MSIX
packaging MSBuild target, and validates the output.
...
try {
    ...
    # WPF requires a platform-specific RID — not AnyCPU
    # ------------------------------------------------------------------
    ...
    Assert-MsixCreated -Directory $ctx.SampleDir -Description "wpf-app MSIX package"
