...
- **Packaging any Windows app** â€” GUI apps, console apps, CLI tools, services, or background processes
...
2. **`appxmanifest.xml`** â€” from `winapp init` or `winapp manifest generate`
3. **Certificate** (optional) â€” `devcert.pfx` from `winapp cert generate` for signing
...
```powershell
# Package from build output â€” manifest auto-detected from current dir or input folder
...
1. **Locates `appxmanifest.xml`** â€” looks in input folder, then current directory (or uses `--manifest`)
2. **Copies manifest + assets** into a staging layout alongside your app files
3. **Generates `resources.pri`** â€” Package Resource Index for UWP-style resource lookup (skip with `--skip-pri`)
4. **Runs `makeappx pack`** â€” creates the `.msix` package file
5. **Signs the package** (if `--cert` provided) â€” calls `signtool` with your certificate
...
2. **Package** â€” `winapp package <build-output> --cert ./devcert.pfx`
3. **Trust cert** (first time) â€” `winapp cert install ./devcert.pfx` (admin)
4. **Install** â€” double-click the `.msix` or `Add-AppxPackage ./myapp.msix`
...
- The `package` command aliases to `pack` â€” both work identically
- `appxmanifest.xml` Publisher must match the certificate publisher â€” use `winapp cert generate --manifest` to ensure they match
...
- The `--executable` flag overrides the entry point in the manifest â€” useful when your exe name differs from what's in `appxmanifest.xml`
...
| "Publisher mismatch" | Cert publisher â‰  manifest publisher | Regenerate cert with `winapp cert generate --manifest`, or edit manifest |
