...
- Use `--frames-dir` when an agent must inspect intermediate states without a video decoder. It requires a timed recording, `--fps 1..30`, and `duration-sec × fps <= 18,000`, and writes `manifest.json`, `frames.ndjson`, and changed JPEGs under `frames/`; exact duplicate samples reuse the prior JPEG. Frame mode defaults `--max-edge` to 1280; when specified, use 64-4096 (`0` is not supported). Very large, near-square output dimensions that exceed the 256 MiB frame-pipeline memory budget are rejected with guidance to lower `--max-edge`. Frame data is capped at 1 GiB; reaching the cap publishes the valid indexed prefix as `truncated` while the MP4 finishes. Use monotonic `elapsedMs` in the index to bound UI transitions.
