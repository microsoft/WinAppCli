...
When adding or changing public facing features, ensure all documentation is also updated. Places to update (but not limitted to):
...
If a feature is big enough and requires it's own docs page, add it under docs\
...
- `.github/plugin/skills/winapp-cli/*/SKILL.md` — Copilot CLI plugin skills (also embedded in CLI binary via csproj link)
