...
- Long literal text on `--via send-input` is **auto-throttled**: the text is split into small character chunks injected one `SendInput` at a time with a brief pause between them, so the target can drain its input queue and every character lands. A single unbroken burst overruns the queue and silently drops characters even though the command reports success. This adds a little latency only to genuinely long text — short text, key names, and modifier combos inject in one call with no added delay. For large bulk text, prefer `set-value` (atomic, no keystrokes or foreground needed) on controls that support it.
