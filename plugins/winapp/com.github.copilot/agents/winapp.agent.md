...
- `--dry-run` — (project mode) validate the build/publish plan, restored Native AOT assets, requested x64/ARM64 linker, and Windows SDK without restoring, building, publishing, registering, or launching
...
**Requires:** Folder mode — built app output directory + `appxmanifest.xml`. Project mode — a `.csproj`/`.sln`/`.slnx` (or directory containing one) + .NET SDK 8.0.100+. Windows Native AOT publishing also requires Desktop development with C++. Set `PublishAot` in the runnable app project rather than passing it globally with `-p`. If the standard Visual Studio Installer directory contains `vswhere.exe`, WinApp adds it only to the publish process's `PATH`; WinApp does not install the C++ workload.
