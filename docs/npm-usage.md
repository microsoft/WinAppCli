...
| `outputAppxDirectory` | `string \| undefined` | No | Output directory for the loose layout package. If not specified, a directory named AppX inside the input directory will be used. Files winapp staged there are kept matching the build, so a file your app no longer contains is removed on the next run; files winapp did not put there are left alone. |
