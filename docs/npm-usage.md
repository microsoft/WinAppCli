...
| `outputAppxDirectory` | `string \| undefined` | No | Output directory for the loose layout package. If not specified, a directory named AppX inside the input directory will be used. The directory is kept matching the build, so files the app no longer contains are removed from it. |
