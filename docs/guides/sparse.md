...
- `appxmanifest.xml` — a sparse manifest with `uap10:AllowExternalContent="true"`, `ProcessorArchitecture="neutral"`, a `win32App` application, and the exe name filled into `Executable`.
...
- `winapp pack <file>` only accepts a manifest that declares `uap10:AllowExternalContent="true"`. Generate one with `winapp init --exe <exe> --sparse`, or pass an input *folder* to build a full MSIX.
