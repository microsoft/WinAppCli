...
Project mode supports both **packaged** and **unpackaged** WinUI apps — it detects which from the project's `WindowsPackageType` and installs the matching-architecture Windows App Runtime automatically. To force an unpackaged run of a packaged project, add `-p WindowsPackageType=None`. The target architecture is normally conveyed by the runtime identifier alone (`--arch` → `-r win-<arch>`); when the project — and every project it references — declares a `<Platforms>` that includes that arch, winapp also passes the matching `-p:Platform=<arch>` so builds that older Windows App SDK versions reject under the default `Platform=AnyCPU` (self-contained and packaged builds) succeed. **Multi-project apps** (an app that references other projects, e.g. a solution split into class libraries) build correctly too: winapp negotiates each project reference's own platform automatically, so a project referencing an `AnyCPU`/`netstandard2.0` library no longer fails with `CS0006` "metadata file could not be found" — and because the explicit `Platform` above is only injected when the *whole* reference closure declares the arch, a no-`<Platforms>` library reference is never forced onto a platform it can't build. The `dotnet build` output **streams live** while it builds (winapp prints the exact `dotnet build …` invocation before the output, and warnings on a successful build stay visible); in a real interactive terminal winapp hands the console to dotnet so its native terminal-logger renders the live build (each warning shown once), while agents/CI and redirected output get plain streamed lines. Add `--verbose` to also see winapp's own build decision traces (the dotnet build stays at `minimal`; use `--trace` to raise dotnet to `-v normal`). Project mode requires the .NET SDK 8.0.100 or newer. See [`winapp run` in the usage reference](../usage.md#project-mode-net-sdk-projects) for the full option list.

> **No Windows SDK installed?** C#/WinRT authoring projects (anything using `Microsoft.Windows.CsWinRT`, WinUI control libraries that author winmds, etc.) normally need a registered Windows SDK to build. When project mode detects that **no** Windows SDK is registered on the host (clean CI, containers, SDK-less dev boxes), it automatically points cswinrt at the winmds from the auto-restored `Microsoft.Windows.SDK.NET.Ref` NuGet ref pack so the build still succeeds — no action needed. This is a temporary shim: it does nothing when an SDK is installed or when you set `-p CsWinRTWindowsMetadata=…` yourself, and it silently steps aside if the ref pack isn't available.
