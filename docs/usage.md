...
The built-in corpus is refreshed at release time on a best-effort basis: if that refresh can't reach GitHub the previously committed corpus ships again, so it can be more than one release behind upstream. When results are served from it, `find-ui` says so on stderr and `--json` output carries `"corpus": "embedded"` (other values: `"network"` for a fresh fetch, `"cache"` for the local cache).
