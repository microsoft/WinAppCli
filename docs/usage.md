...
- `--output-appx-directory <path>` - Output directory for the loose layout package (default: `AppX` inside the input folder directory). winapp keeps the files it stages there matching the build, so a file your app no longer contains is removed on the next run. Files winapp did not put there are never removed, so it is safe to point this at a directory that already holds something. If the directory contains the build output, winapp warns and removes nothing, rather than deleting build artifacts the package does not include. The path may not be, or sit under, a symbolic link or junction.
