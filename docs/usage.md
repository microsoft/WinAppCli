...
When invoked via the `@microsoft/winappcli` npm package (i.e. `npx winapp init` inside a Node / Electron project), `init` adds an interactive **bindings prompt**:
...
JS/TS bindings are configured by declaring a `"winapp": { "jsBindings": {...} }` namespace in **`package.json`** and generated by `winapp init` (first run) or `winapp restore` (subsequent runs). There is no separate `node jsbindings` sub-command — the flow is unified with the rest of the workspace lifecycle:
...
- Reads the existing `winapp.jsBindings` namespace from `package.json` (no mutation)
- Resolves winmds via NuGet cache walk + transitive-deps expansion
- Spawns `@microsoft/dynwinrt-codegen` to emit `.js` + `.d.ts` into the configured `output` directory (default `bindings/`)
