...
- `send-input` produces fully real input (modifiers visible to `GetAsyncKeyState`, fires low-level hooks) but goes to whatever window is foreground and is **blocked by UIPI when injecting from an elevated process into a lower-integrity (AppContainer/AppX) target**. If `send-input` reports a failure, the target is likely elevated or an AppX app — use `post-message`, or run the CLI at a matching integrity level. As a safety guard, `send-input` verifies the target window is actually in the foreground immediately before injecting and **fails (`foreground_not_target`) rather than typing into the wrong window** if focus could not be brought to it — focus or click the window first. Because a long literal-text payload is injected over many paced `SendInput` calls (see auto-throttling below), the foreground is re-verified before **each continuation chunk** and the send aborts with the same `foreground_not_target` error if focus leaves the target mid-injection — so a focus change partway through can't spray the remaining text into another window (a focus-changing chord such as `alt+tab` is exempt and is never treated as drift). On a **locked or secure desktop** it instead fails with **`no_interactive_desktop`** (no foreground window exists to inject into) — unlock the session, or use a UIA-pattern verb (`set-value`, `invoke`).
- **Long literal text is auto-throttled.** Text sent via `send-input` is split into small character chunks, each injected as its own `SendInput` with a brief pause between the chunks of that one run, so the target can drain its input queue and **every character lands**. A single unbroken burst overruns the queue and silently drops characters even though the command reports success (issue #657). The pacing is scoped to a single long text run — short text, key names, and modifier combos (including sequences like `ctrl+a delete`) still inject with no added delay, and the command emits an informational warning when a payload is large enough to be throttled. For large bulk text, prefer **`set-value`** (atomic, no keystrokes or foreground needed) on controls that support it.
