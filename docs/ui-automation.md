...
- **System-reserved combos** (`win+l`, `win+r`, `ctrl+shift+esc`, `ctrl+alt+del`, `alt+tab`, `alt+f4`, `ctrl+esc`, lone `win`/`printscreen`, …) act on the OS/shell rather than just the target when sent OS-wide. `send-input` **rejects them by default** (errors with `invalid_arguments` and sends nothing) because injecting them at the OS level has effects well beyond the target window (e.g. `win+l` would lock the session). Pass **`--allow-system-keys`** to opt in — this is what lets you drive a global hotkey such as PowerToys' `win+shift+v` (the global low-level hook watches the OS-wide input stream, so the injected combo fires it). Windows still blocks secure sequences like `ctrl+alt+del` (SAS) from injected input regardless of the flag. Alternatively, to deliver one to a specific window use `--via post-message`, which is window-scoped and unaffected (a posted `win+l` is harmless, though a posted `alt+f4` still closes the target window).
