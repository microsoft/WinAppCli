...
| **Capture debug output** | `winapp run .\build\Debug --debug-output` | Captures `OutputDebugString`; on crash, writes minidump and runs CDB analysis. **Blocks other debuggers** (one debugger per process) |
