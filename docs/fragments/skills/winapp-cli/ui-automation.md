...
## JSON envelope (v0.3.1+)

The `--json` output for several `ui` subcommands was reshaped in v0.3.1.
Generate parsers against these shapes — pre-0.3.1 parsers will silently break
because most fields were renamed, removed, or moved into envelopes. The
internal `id`, `parentSelector`, and `windowHandle` fields are **scrubbed**
from results — use `selector` as the only public element handle.

### `ui inspect --json`

Elements are now nested under `windows[]` rather than emitted at the top level:
...
Both return matching elements using the same element shape as `ui inspect`
(`selector`, `name`, `controlType`, `children`, etc.). Each match may also
include an `invokableAncestor` field — itself an element-shaped object —
pointing to the nearest parent that supports `InvokePattern` (useful when a
search hits a non-invokable element like a label inside a button).
...
Other `ui` subcommands (`list-windows`, `click`, `screenshot`, etc.) keep
their pre-0.3.1 JSON output.
