...
| Capture OutputDebugString + crash dump | `winapp run .\build\Debug --debug-output` | On crash, writes minidump and shows faulting source/line. **Blocks other debuggers** — use `--no-launch` if you need VS Code/WinDbg |
