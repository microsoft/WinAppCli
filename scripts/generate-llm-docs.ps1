...
# Generate each skill
$SkillNames = @("setup", "package", "identity", "signing", "manifest", "troubleshoot", "find-ui", "frameworks", "ui-automation")
$SkillDescriptions = @{
    ...
    "find-ui"      = "Search WinUI 3 controls and samples for a working code example. Use when building a WinUI 3 UI and you need to discover which control fits an intent (e.g. 'tabbed layout', 'a card with an image and title', 'swipeable list rows') and get real XAML + C# from the WinUI Gallery or Windows Community Toolkit. WinUI-only — not WPF/WinForms. Distinct from 'winapp ui', which automates a running app's UI."
