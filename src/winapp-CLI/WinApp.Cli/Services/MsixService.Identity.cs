...
internal partial class MsixService
{
    ...
    /// <summary>
    /// Copies files to the AppX layout directory using the .build.appxrecipe file, then removes
    /// layout files the recipe no longer lists.
    ...
    /// </summary>
    /// <remarks>
    /// The layout directory is winapp's own artifact — it is created here and MSBuild never sees
    /// it, so <c>dotnet clean</c> does not empty it. Copying without pruning therefore turned the
    /// layout into an ever-growing union of every build ever materialized into it: a source file
    /// deleted from the project stayed in the package, and (because an execution target mirrors
    /// this layout exactly) stayed in the guest too. Reconciling against the recipe — the same
    /// authoritative file list MSBuild packages from — keeps the layout equal to the build.
    /// </remarks>
    private static async Task CopyFilesFromRecipeAsync(FileInfo recipeFile, DirectoryInfo outputDir, TaskContext taskContext, CancellationToken cancellationToken)
    {
        ...
        // Every PackagePath the recipe describes, including entries whose source file is missing.
        // A missing source means "we could not refresh this one", not "it does not belong" — so it
        // is recorded here and whatever the layout already holds for it is left alone.
        ...
        foreach (var entry in manifestEntries)
        {
            ...
            recipePaths.Add(NormalizePackagePath(packagePath));
        ...
        // Prune last, so a copy that threw part-way leaves the previous layout as complete as it
        // was rather than deleting from a layout that is no longer being refreshed.
        //
        // The recipe lists only what goes into the package, so it is authoritative for the layout
        // but NOT for the build output it is copied from: a .pdb, .deps.json or the recipe itself
        // are absent from it and would be deleted as "stale". If someone points the layout at the
        // build output (or at a folder above it), pruning would therefore destroy the build, so
        // this reconciles only when the two are genuinely separate directories.
        var removed = 0;
        if (IsPathInsideDirectory(recipeFile.DirectoryName, outputDir.FullName))
        {
        ...
        {
            removed = PruneLayoutToRecipe(outputDir, recipePaths, taskContext);
        }

        taskContext.AddDebugMessage($"{UiSymbols.Check} AppX layout from recipe: {copied} copied, {skipped} unchanged, {removed} removed");
    }
    ...
    /// <summary>
    /// Deletes files in <paramref name="outputDir"/> that <paramref name="recipePaths"/> does not
    /// describe, then removes any directory those deletions emptied. Returns the number of files removed.
    /// </summary>
    /// <remarks>
    /// Deliberately conservative in three ways. An empty recipe prunes nothing, because a recipe that
    /// described no files is evidence of a bad read, not of an empty app. Directory reparse points are
    /// never descended into, so a junction someone placed in the layout cannot make this delete files
    /// outside it. And a file that cannot be deleted — held open by a running app, or by another winapp
    /// run racing on the same layout — is reported and skipped rather than failing a materialization
    /// that has otherwise already succeeded.
    /// </remarks>
    private static int PruneLayoutToRecipe(DirectoryInfo outputDir, HashSet<string> recipePaths, TaskContext taskContext)
    {
        if (recipePaths.Count == 0)
        {
            taskContext.AddDebugMessage($"{UiSymbols.Warning} Recipe listed no files; leaving the existing layout untouched.");
            return 0;
        }
        ...
        foreach (var file in EnumerateLayoutFiles(outputDir))
        {
            ...
            if (recipePaths.Contains(relativePath) || ProtectedLayoutFiles.Contains(relativePath))
            {
                continue;
            }
