...
internal sealed class FakeControlsSearchService : IControlsSearchService
{
    ...
    public Task<SearchEngine> GetEngineAsync(bool forceRefresh = false, bool allowCoreOnly = false, Action<string>? onFetchStarting = null, CancellationToken cancellationToken = default)
    {
        GetEngineCalls++;
...
internal sealed class FakeSearchProvider : ISearchProvider
{
    ...
    public Task<ProviderData> LoadAsync(bool forceRefresh = false, Action<string>? onFetchStarting = null, CancellationToken cancellationToken = default)
    {
        LoadCalls++;
