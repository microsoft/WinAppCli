...
public class MsixServiceIdentityTests : BaseCommandTests
{
    ...
    [TestMethod]
    public async Task AddLooseLayoutIdentityAsync_PublishRefusesUnrelatedDevelopmentRegistration()
    {
       var projectDir = _tempDirectory.CreateSubdirectory("publish-project");
       var unrelatedDir = _tempDirectory.CreateSubdirectory("other-project");
       var project = new FileInfo(Path.Combine(projectDir.FullName, "App.csproj"));
       await File.WriteAllTextAsync(project.FullName, "<Project />", TestContext.CancellationToken);
       var manifest = new FileInfo(Path.Combine(projectDir.FullName, "AppxManifest.xml"));
       await File.WriteAllTextAsync(manifest.FullName, BuildMSBuildManifest(), TestContext.CancellationToken);
       await File.WriteAllTextAsync(
           Path.Combine(projectDir.FullName, "TestApp.exe"),
           "exe",
           TestContext.CancellationToken);
       var output = new DirectoryInfo(Path.Combine(_tempDirectory.FullName, "selected-stage"));
       _fakeRegistration.FakeDevPackages =
       [
           new DevPackageInfo(
               "TestApp_1.0.0.0_x64__unrelated",
               "TestApp",
               "1.0.0.0",
               unrelatedDir.FullName,
       ...
       StringAssert.Contains(error.Message, "unrelated location");
       Assert.AreEqual(0, _fakeRegistration.UnregisterByFullNameCalls.Count);
       Assert.AreEqual(0, _fakeRegistration.RegisterLooseLayoutCalls.Count);
    }

    [TestMethod]
    public async Task AddLooseLayoutIdentityAsync_PublishRefusesProductionRegistrationInsideProject()
    {
       var projectDir = _tempDirectory.CreateSubdirectory("production-conflict-project");
       var project = new FileInfo(Path.Combine(projectDir.FullName, "App.csproj"));
       await File.WriteAllTextAsync(project.FullName, "<Project />", TestContext.CancellationToken);
       var manifest = new FileInfo(Path.Combine(projectDir.FullName, "AppxManifest.xml"));
       await File.WriteAllTextAsync(manifest.FullName, BuildMSBuildManifest(), TestContext.CancellationToken);
       await File.WriteAllTextAsync(
           ...
           "exe",
           TestContext.CancellationToken);
       var output = new DirectoryInfo(Path.Combine(projectDir.FullName, "AppX"));
       _fakeRegistration.FakeDevPackages =
       [
           new DevPackageInfo(
               "TestApp_1.0.0.0_x64__production",
               "TestApp",
               "1.0.0.0",
               projectDir.FullName,
       ...
       StringAssert.Contains(error.Message, "will not replace production registrations");
       Assert.AreEqual(0, _fakeRegistration.UnregisterByFullNameCalls.Count);
       Assert.AreEqual(0, _fakeRegistration.RegisterLooseLayoutCalls.Count);
    }

    [TestMethod]
    public async Task AddLooseLayoutIdentityAsync_PublishFailsClosedWhenRegistrationInspectionFails()
    {
       var projectDir = _tempDirectory.CreateSubdirectory("inspection-failure-project");
       var project = new FileInfo(Path.Combine(projectDir.FullName, "App.csproj"));
       await File.WriteAllTextAsync(project.FullName, "<Project />", TestContext.CancellationToken);
       var manifest = new FileInfo(Path.Combine(projectDir.FullName, "AppxManifest.xml"));
       await File.WriteAllTextAsync(manifest.FullName, BuildMSBuildManifest(), TestContext.CancellationToken);
       await File.WriteAllTextAsync(
           ...
           TestContext.CancellationToken);
       var output = new DirectoryInfo(Path.Combine(projectDir.FullName, "AppX"));
       _fakeRegistration.FindDevPackagesThrows =
           new InvalidDataException("package inventory unavailable");
       ...
       StringAssert.Contains(error.Message, "could not safely inspect or replace");
       Assert.AreEqual(0, _fakeRegistration.RegisterLooseLayoutCalls.Count);
    }
