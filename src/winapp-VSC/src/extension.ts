...
class WinAppDebugAdapterFactory implements vscode.DebugAdapterDescriptorFactory {
	...
	async createDebugAdapterDescriptor(
		...
		try {
			// Resolve the manifest path: use an explicit path if provided, otherwise search via glob
			let manifest: string;
			if (config.manifest) {
				// Exact manifest path provided — use it directly
				manifest = config.manifest;
			} else {
				// Search for AppxManifest.xml in build output using glob (bypasses .gitignore).
				// The default pattern matches at any depth, including the workspace root.
				...
				// Filter out manifests inside AppX or .winapp folders (created by winapp run/debug)
				const matches = allMatches.filter(m => {
					const parts = m.split(path.sep);
					return !parts.includes('AppX') && !parts.includes('.winapp');
				});

				if (matches.length === 0) {
					throw new Error(`No manifest found matching "${searchPattern}". Build your project first, set "manifest" to an explicit path, or update "buildOutputManifest" in launch.json.`);
			...
			// inputFolder is the directory containing the app binaries passed to winapp run.
