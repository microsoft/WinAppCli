...
export function getWebviewContent(webview: vscode.Webview, nonce: string): string {
    ...
    const familyOptions = DEVICE_FAMILY_OPTIONS.map(f => `<option value="${f}">${f}</option>`).join('');
...
<body>
    ...
    <div class="tab-content active" id="tab-identity" role="tabpanel">
        ...
        <div class="form-group" data-field="identity.version">
            ...
            <div class="description">Major.Minor.Build.Revision format</div>
            <div class="validation-msg"></div>
        </div>
    ...
    <div class="tab-content" id="tab-dependencies" role="tabpanel">
        ...
        <button class="btn" id="add-target-family">+ Add Target Device Family</button>

        <div class="section-header" style="margin-top:24px;">Package Dependencies</div>
    ...
    (function() {
        ...
        // ─── Add/Remove target device family ────────────────
        document.getElementById('add-target-family').addEventListener('click', () => {
            vscode.postMessage({
                type: 'addTargetDeviceFamily',
                family: { name: 'Windows.Desktop', minVersion: '10.0.17763.0', maxVersionTested: '10.0.26100.0' }
            });
        });
            ...
            families.forEach((fam, idx) => {
                ...
                item.innerHTML = \`
                    <div class="item-header">
                        <span class="item-title">Target Device Family #\${idx + 1}</span>
                        <button class="btn btn-danger btn-sm remove-family" data-index="\${idx}">Remove</button>
                    </div>
                    <div class="form-group" data-field="dependencies.targetDeviceFamily.\${idx}.name">
                        <label>Name</label>
                        <select data-section="dependencies" data-field-name="targetDeviceFamily.name" data-index="\${idx}">
                            ${familyOptions}
                        </select>
                        <div class="validation-msg"></div>
                    </div>
        ...
        function renderPackageDependencies(deps) {
            ...
            deps.forEach((dep, idx) => {
                ...
                item.innerHTML = \`
                    <div class="item-header">
                        <span class="item-title">Package Dependency #\${idx + 1}</span>
        ...
        function renderApplications(apps) {
            ...
            apps.forEach((app, idx) => {
                ...
                if (app.extensions && app.extensions.length > 0) {
                    extHtml = '<div style="margin-top:8px;"><label style="font-weight:600;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;">Extensions</label><ul class="ext-list">' +
                        app.extensions.map(e => '<li>' + escapeHtml(e) + '</li>').join('') + '</ul></div>';
                }
                ...
                card.innerHTML = \`
                    ...
                    <div class="subsection">
                        <div class="subsection-title">Visual Elements</div>
                    ...
                    <div class="extensions-note">
                        To edit Extensions or other application details, use the text editor (Right-click → Open With → Text Editor).
                    </div>
                \`;
